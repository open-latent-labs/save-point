...
async def embed_chunks(chunks: list[str]) -> list[list[float]]:
    ...
    return vectors #이거 받은 백엔드는 나중에 여기에 값이 제대로 있는지 테스트 후 아래 저장 함수 호출

# 벡터 저장
async def store_vectors(chunks: list[str], vectors: list[list[float]], metadata: ChunkMetadata,) -> None:
