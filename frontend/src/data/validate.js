...
export function validateSignup({ name, username, email, password, confirm }) {
  const e = {};
  if (!name.trim()) e.name = "이름을 입력해 주세요.";
  if (!username.trim()) e.username = "아이디를 입력해 주세요.";
  else if (username.trim().length < 2) e.username = "아이디는 2자 이상이어야 해요.";
  if (!email.trim()) e.email = "이메일을 입력해 주세요.";
  else if (!isEmail(email)) e.email = "올바른 이메일 형식이 아니에요.";
  if (!password) e.password = "비밀번호를 입력해 주세요.";
